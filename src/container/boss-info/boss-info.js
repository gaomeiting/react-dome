		...
		return (
			<div>
				...
				<WingBlank>
					...
					<InputItem type="text" placeholder="请输入招聘职位" onChange={(v) => this.changeForm('job' , v)} clear  >招聘职位</InputItem>
					<InputItem type="text" placeholder="请输入公司名称" onChange={(v) => this.changeForm('name' , v)} clear >公司名称</InputItem>
					<InputItem type="money" moneyKeyboardAlign="left" placeholder="请输入职位薪资" onChange={(v) => this.changeForm('salary' , v)} clear >职位薪资</InputItem>
					<TextareaItem type="text" placeholder="请输入职位要求" title="职位要求" rows="4" count="150" onChange={(v) => this.changeForm('desc' , v)} ></TextareaItem>
					<WhiteSpace />
